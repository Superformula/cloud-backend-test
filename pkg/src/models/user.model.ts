...
export default class UserModel extends BaseModel {
	...
	async loadItem(dynamoConn: AWS.DynamoDB.DocumentClient): Promise<void> {
		...
		else if (userData && Array.isArray(userData.Items) && userData.Items.length === 1)
		{
			const item = userData.Items[0]!
		...
		else throw new Error(`a DynamoDb table CANNOT have the same pk and sk for more than one items, please check your data`)
	}
